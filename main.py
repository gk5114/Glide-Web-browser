    ...
    def scrape_url(self, url):
        try:
            ...
            if response.status_code == 200:
                ...
                self.display_main_elements(soup)
            ...
            QMessageBox.warning(self, "Scraping Error", f"Error fetching webpage: {str(e)}")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"An error occurred while scraping the webpage: {str(e)}")

    def display_main_elements(self, soup):
        # Displaying main elements of the webpage
        main_elements = soup.find_all(
            ['p', 'h1', 'h2', 'h3', 'a'])  # You can customize this list according to your needs
        main_content = "\n".join([element.text.strip() for element in main_elements])
        QMessageBox.information(self, "Main Elements", main_content)
